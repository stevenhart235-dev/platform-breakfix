...
$approvedSha = 'd042fa4e9a3b8ef975554fa56b7c9a4b61547e54'
$approvedSourceSha = 'F03C4401552FB6F9F9BE65DB813127B8ECF020A1089A8889064F95C4A0B4D866'
$approvedLifecycleSourceSha = 'F324164D951F16D0EEC6ED871DB4BFA787D12D8C392D1A88E22F0047ED43D85B'
...
$required=@('SchemaVersion','Name','RepositoryUrl','Version','Tag','CommitSha','SourcePath','SourceSha256','LifecycleStatusSourcePath','LifecycleStatusSourceSha256','ContractMajorVersion');$fields=@($lock.psobject.Properties.Name)
...
Assert-True ($lock.Version-ceq'0.2.0'-and$lock.Tag-ceq'v0.2.0'-and$lock.CommitSha-ceq$approvedSha-and$lock.ContractMajorVersion-eq0) 'Foundation lock version, tag, commit, or contract major is invalid.'
Assert-True ($lock.SourcePath-ceq'src/DeterministicSelection.ps1'-and$lock.SourceSha256-ceq$approvedSourceSha-and$lock.LifecycleStatusSourcePath-ceq'src/LifecycleStatus.ps1'-and$lock.LifecycleStatusSourceSha256-ceq$approvedLifecycleSourceSha) 'Foundation lock source identity is invalid.'
...
$lifecycleSourcePath=Join-Path $submodulePath $lock.LifecycleStatusSourcePath;Assert-True (Test-Path -LiteralPath $lifecycleSourcePath -PathType Leaf) 'Locked lifecycle-status source is absent.'
$lifecycleSourceSha=Get-GitBlobSha256 -Repository $submodulePath -Revision $approvedSha -Path $lock.LifecycleStatusSourcePath;Assert-True ($lifecycleSourceSha-ceq$approvedLifecycleSourceSha) 'Lifecycle-status Git blob SHA-256 disagrees with the lock.'
...
$t=$null;$parseErrors=$null;[void][Management.Automation.Language.Parser]::ParseFile($lifecycleSourcePath,[ref]$t,[ref]$parseErrors);Assert-True (@($parseErrors).Count-eq0) 'Pinned lifecycle-status source does not parse.'
...
$operationsSource=Get-Content -Raw -LiteralPath (Join-Path $RepositoryRoot 'scripts/BreakfixOperations.ps1');$lifecycleDependency='external/cluster-foundation/src/LifecycleStatus.ps1'
Assert-True ($operationsSource-match[regex]::Escape($lifecycleDependency)-and$operationsSource-match'Resolve-LifecycleStatus'-and$operationsSource-notmatch'ConvertTo-BreakfixTimestamp') 'Breakfix status does not delegate exclusively to the pinned lifecycle normalizer.'
$lifecycleImplementations=@(Get-ChildItem (Join-Path $RepositoryRoot 'scripts'),(Join-Path $RepositoryRoot 'providers'),(Join-Path $submodulePath 'src') -Recurse -File -Filter '*.ps1'|Where-Object{(Get-Content -Raw $_.FullName)-match'function Resolve-LifecycleStatus'})
Assert-True ($lifecycleImplementations.Count-eq1-and$lifecycleImplementations[0].FullName-ceq$lifecycleSourcePath) 'Exactly one reachable lifecycle-status implementation was not found.'
$lifecycleExecutable=Get-Content -Raw $lifecycleSourcePath
Assert-True ($lifecycleExecutable-notmatch'platform-breakfix|BreakfixOperations|Lab-Aks|Azure|AKS|AWS|EKS') 'Lifecycle-status foundation source leaks consumer or provider semantics.'
